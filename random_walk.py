...
# Final static plot with the results of the whole simulation
fig, axes = plt.subplots(6,1)
if len(np.concatenate(all_spikes[:][0], axis=None)) != len(np.concatenate(all_spikes[:][1], axis=None)):
	print(np.concatenate(all_spikes[:][0], axis=None), np.concatenate(all_spikes[:][1], axis=None))
axes[0].scatter(np.concatenate(all_spikes[:][0], axis=None), np.concatenate(all_spikes[:][1], axis=None),s=4)
axes[0].set_xlabel("time [ms]")
axes[0].set_xlim((0, simtime))
axes[0].set_ylim((0, width*height))
axes[0].set_title("Filter High")
if len(np.concatenate(all_spikes[:][2], axis=None)) != len(np.concatenate(all_spikes[:][3], axis=None)):
	print(np.concatenate(all_spikes[:][2], axis=None), np.concatenate(all_spikes[:][3], axis=None))
axes[1].scatter(np.concatenate(all_spikes[:][2], axis=None), np.concatenate(all_spikes[:][3], axis=None),s=4)
axes[1].set_xlabel("time [ms]")
axes[1].set_xlim((0, simtime))
axes[1].set_ylim((0, width*height))
axes[1].set_title("Filter Low")
axes[2].vlines(np.concatenate(all_spikes[:][4], axis=None), ymin=0, ymax=1, color="red", linestyle="--")
axes[2].set_xlabel("time [ms]")
axes[2].set_xlim((0, simtime))
axes[2].set_title("Up neuron")
axes[3].vlines(np.concatenate(all_spikes[:][5], axis=None), ymin=0, ymax=1, color="red", linestyle="--")
axes[3].set_xlabel("time [ms]")
axes[3].set_xlim((0, simtime))
axes[3].set_title("Down neuron")
axes[4].vlines(np.concatenate(all_spikes[:][6], axis=None), ymin=0, ymax=1, color="red", linestyle="--")
axes[4].set_xlabel("time [ms]")
axes[4].set_xlim((0, simtime))
axes[4].set_title("Left neuron")
axes[5].vlines(np.concatenate(all_spikes[:][7], axis=None), ymin=0, ymax=1, color="red", linestyle="--")
axes[5].set_xlabel("time [ms]")
axes[5].set_xlim((0, simtime))
axes[5].set_title("Right neuron")
plt.tight_layout()
plt.savefig("last_test.svg", dpi=150)